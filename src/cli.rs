...
fn handle_score(args: ScoreArgs) -> Result<()> {
    ...
    let syntax_valid = { let mut lang = detect_language(Path::new(&args.file))?; let parse_result = lang.parse(&content); lang.is_valid(&parse_result) };
    let compile_success = { let result = crate::compile::compile_check(Path::new(&args.file), "rs", 30)?; result.success };
...
fn handle_index(args: IndexArgs) -> Result<()> {
    let kg = crate::knowledge::build_project_index(Path::new("."));
    if let Some(ref symbol) = args.callers {
        let callers = kg.callers_of(symbol);
        if args.json {
            println!("{}", serde_json::to_string(&callers)?);
        } else {
            if callers.is_empty() { println!("No callers found for '{}'", symbol); }
            else { for caller in &callers { println!("{}:{}", caller.caller_file, caller.caller_line); } }
        }
    } else {
        println!("Knowledge graph built with {} symbols and {} call edges.", kg.symbols.len(), kg.call_edges.len());
        if args.json { println!("{}", serde_json::to_string(&kg)?); }
    }
    Ok(())
}
