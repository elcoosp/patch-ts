...
use std::path::Path;
use super::types::*;
use crate::render_md::render_gate_report_md;
pub fn handle_gate(args: GateArgs) -> Result<()> {
    ...
    if args.markdown {
        let md = render_gate_report_md(&result.stages, result.passed);
        crate::render_md::print_md(&md);
    } else if args.json { println!("{}", serde_json::to_string(&result)?); }
