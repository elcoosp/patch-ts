...
pub fn balance_file(
    ...
    if errors.is_empty() {
        if language.is_valid(&parse_result) {
            if dry_run { println!("File is already valid; no changes needed."); }
    ...
    let search_result = minimum_cost_repair(&repaired_content, &errors, language, max_cost);
    ...
    let balance_actions: Vec<BalanceAction> = actions.iter().map(|action| {
        match action {
            RepairAction::Insert { ch, pos } => {
                ...
                BalanceAction { action_type: "insert".to_string(), delimiter: *ch, line, column: col, message: format!("Inserted missing '{}'", ch) }
            }
            RepairAction::Delete { start, end: _ } => {
                ...
                BalanceAction { action_type: "remove".to_string(), delimiter, line, column: col, message: format!("Removed extra '{}'", delimiter) }
            }
        }
    }).collect();

    if dry_run { println!("{}", patched_content); } else { fs::write(file_path, &patched_content)?; }

    Ok(BalanceResult { success: true, actions: balance_actions, rolled_back: vec![], error: None })
}
...
fn find_delimiter_errors_in_function(
    ...
    let rust_lang = language.as_any_mut().downcast_mut::<crate::ast::RustLanguage>()
        .ok_or_else(|| anyhow::anyhow!("Function scoping only supported for Rust"))?;
    let source = parse_result.text();
    let (start_byte, end_byte) = rust_lang.find_function_body_range(source, function_name)
    ...
    let filtered: Vec<DelimiterError> = all_errors.into_iter()
        .filter(|e| { let span = e.span(); span.start_byte <= end_byte && span.end_byte >= start_byte })
        .collect();
    if filtered.is_empty() { anyhow::bail!("No delimiter errors found in function '{}'", function_name); }
    Ok(filtered)
}

pub fn quick_balance(content: &str, language: &mut dyn Language) -> Option<String> {
    let parse_result = language.parse(content);
    if language.is_valid(&parse_result) { return Some(content.to_string()); }
...
pub fn balance_files(
    ...
    files.iter().map(|file| balance_file(file, function_name, dry_run, language, None, 10)).collect()
}
